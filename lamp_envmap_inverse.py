...
sensor_cam1 = {
    ...
    'film': { 'type': 'hdrfilm', 'width': int(1985/4), 'height': int(1089/4), 'rfilter': {'type':'gaussian'} },
...
keys = ['my_envmap.data', 'my_envmap.scale']
...
params.keep(['my_envmap.data', 'my_envmap.scale'])
params['my_envmap.data'] = dr.full(TensorXf, 0.5, (128*4,256*4,3))
...
for it in tqdm(range(iterations)):
    ...
    for i in range(len(sensors)):
        ...
        for j in range(len(refs[i])):
            ...
            total_loss += l1_loss(image*mask_ref[0], refs[i][j]*mask_ref[0])

            if it % 100 == 0:
                print(f"\nIteration {it:02d}: loss = {total_loss}")
                #print(f"\Luminance {e}")
                #e = dr.exp(opt[f'log_e_{i}_{j}'])
                write_bitmap('png_lamp_envmap_%03i.exr' % it, params['my_envmap.data'])
                write_bitmap(f'png_lamp_out_{it}_{i}_{j}.exr', image)
    ...
    opt.step()
    opt['my_envmap.data'] = dr.clip(opt['my_envmap.data'], 0.0, 10.0)
    params.update(opt)
